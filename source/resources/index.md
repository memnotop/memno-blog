---
title: 资源
date: 2026-05-30 22:14:00
type: resources
---

这里适合整理常用工具、网站、教程、书籍和资料。

<div class="mem-grid">
  <a class="mem-card" href="#工具">
    <h3>工具</h3>
    <p>提升效率的软件、服务和命令行工具。</p>
  </a>
  <a class="mem-card" href="#学习资料">
    <h3>学习资料</h3>
    <p>教程、文档、书籍和长期可读的材料。</p>
  </a>
  <a class="mem-card" href="#资源模板">
    <h3>模板</h3>
    <p>统一资源记录格式，方便长期维护。</p>
  </a>
</div>

## 工具


- [GitHub](https://github.com/)：代码托管和开源项目。
- [Vercel](https://vercel.com/)：静态网站和前端项目部署。
- [MDN Web Docs](https://developer.mozilla.org/)：Web 技术文档。
- [西部数码](https://www.west.cn/)：域名注册与 DNS 解析管理。

## 学习资料

- 编程笔记
- 读书记录
- 教程收藏

## 资源模板

```markdown
- [资源名称](https://example.com/)：一句话说明它解决什么问题。
```
